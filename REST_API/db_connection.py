...
def init():
  ...
  sqlite_create_table = """CREATE TABLE IF NOT EXISTS quotes(
  ...
  for record in ai_quotes:
    cursor.execute("INSERT INTO quotes(author, quote) VALUES(?, ?)", (record['author'], record['quote']))
...
def update(quote_id, author, quote):
  conn = get_db_connection()
  conn.execute("UPDATE quotes SET author=?, quote=? WHERE id=?", (author, quote, quote_id))
  conn.commit()
  conn.close()
  return 201
